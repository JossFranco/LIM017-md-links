...
const chalk = require('chalk');
   ...
   else if(args.length === 1 && !(args.includes('--validate')) && !(args.includes('--stats'))){ 
    mdLinks(args[0], {validate:false})
    .then((result) =>{
    result.forEach(element => console.log(chalk.bold.yellowBright
      `${('href:')} ${(element.href)}\n
       ${('text:')} ${(element.text)}\n
       ${('file:')} ${(element.file)}\n`)) 
   });//href, text, file 
      ...
      .then((result) =>{
        result.forEach(element => console.log(chalk.bold.blueBright
          `${('href:')} ${(element.href)}\n
           ${('text:')} ${(element.text)}\n
           ${('file:')} ${(element.file)}\n
